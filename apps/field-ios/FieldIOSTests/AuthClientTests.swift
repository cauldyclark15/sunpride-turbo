...
final class AuthClientTests: XCTestCase {
    ...
    /// Standard happy backend: sign-in sets `set-auth-token`; token exchange returns a 15-minute JWT.
    ...
    func testSuccessWithoutSetAuthTokenHeaderIsNotASession() async {
        StubURLProtocol.install { _ in .reply(.json(200, ["token": "body-token-must-be-ignored"])) }
        let auth = makeAuth()
        await XCTAssertThrowsMobileError(try await auth.signIn(email: "a@b.c", password: "x"), .invalidResponse)
        XCTAssertFalse(auth.hasSession)
    }
